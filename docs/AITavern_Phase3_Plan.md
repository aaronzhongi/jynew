...
| Q3 | Affection baseline (prose → scalar) | **AMENDED (3C, ai-town-RATIFIED).** Original ("offline call emits baseline beside the dossier") was never built — the T3A.6 pipeline/JSON contract emits no scalar. Final: the affection baseline is derived deterministically at boot from the committed Phase 1 `RelationType` canon (`AffectBaseline.ForRelation`: Enemy −0.7 / Rival −0.4 / Neutral 0 / Ally +0.4 / Friend +0.6; unset edges incl. NPC→player → Neutral 0); no pipeline pass or Grok call — runtime seeds `Value = Baseline` and decay relaxes toward it, never toward 0. (RelationType is the same structured canon driving §4 dossier + the "Your view of X" line; the dropped prose→scalar Grok pass would have been a lossy non-deterministic re-derivation of it.) |
