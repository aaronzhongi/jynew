...
def reduce_character(client: XaiClient, roster_entry: dict,
    ...
    user = (
        ...
        f"10.【MartialNote 归属・强制】某 target 的 martialNote 只能写"
        f"『传主在第{config.CUTOFF_HUI}回前 亲眼见 该 target 本人 施展』"
        f"的招式路数。严禁把传主自己的武功、或把另一人的武功，"
        f"安到该 target 头上。特别地：『借力打力/白驼山内功/灵蛇拳』"
        f"是 欧阳克 的标志武功，绝不可出现在 完颜康(杨康) 或 黄蓉 的"
        f"martialNote（那会以武功指纹变相把二人与欧阳克混为一谈，"
        f"正是规则9所禁）；黄蓉 的 martialNote 只应是 桃花岛路数 + 软猬甲。"
        f"若传主截至第{config.CUTOFF_HUI}回未亲见某 target 出手，"
        f"该 martialNote 留空字符串，不得臆测。\n\n"
        f"{_json_only(schema)}\n\n"
